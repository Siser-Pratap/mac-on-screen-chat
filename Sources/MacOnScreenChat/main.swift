...
// Dry-run the `/command {…}` and `WW:{…}` markers against a sample input.
if let flag = CommandLine.arguments.firstIndex(of: "--markers") {
    SelfTest.markers(CommandLine.arguments.dropFirst(flag + 1).joined(separator: " "))
    exit(0)
}
